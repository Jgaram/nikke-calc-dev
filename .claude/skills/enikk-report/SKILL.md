...
스펙 형식·육성 기본값·운용 조건 표기는 전부 `.claude/skills/report/REPORT.md`가 정본이다 —
**이 문서는 그걸 다시 적지 않는다.**
...
`char-add`를 먼저 돌릴지 판단할 문제다. Claude가 임의로 등록하러 가지 않는다.
...
```bash
python .claude/skills/report/report.py reports/specs/<슬러그>.json --jobs 8
```
...
```bash
python .claude/skills/enikk-report/report_ref.py \
