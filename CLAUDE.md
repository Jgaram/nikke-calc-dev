...
**1층이 아닌 채로 돌린 결과는 언제나 그 사실과 함께 보고한다.** 세 도구 모두 이탈 목록을
출력에 싣는다(`/report`는 접히지 않는 배너, 하네스는 baseline에 박아 FAIL로 잡는다).
**유저에게 답할 때도 그 줄을 그대로 옮긴다** — 총딜만 옮기고 "앨리스 톡톡이가 켜져 있었다"를
빠뜨리면 유저는 기본 스펙 수치로 읽는다.
