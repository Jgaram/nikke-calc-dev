...
| 신데렐라 : 크리스탈 웨이브 | 스킬2 | `저격모드 적용 시` → `event:저격 모드`, `저격모드 해제 시` → `event:state_end:저격 모드`로 파싱했으나, **weapon_change는 `_active`에 등록되지 않아 이 event가 자동 발생하지 않는다**(레드 후드 `event:레드 울프`와 동일 구조의 구현 공백). `디스트로이`·`핀 포인트`·`self_state:저격 모드` 조건 전부가 이 경로에 걸려 있어 `/char-impl`에서 별도 발생 경로가 필요하다. |
