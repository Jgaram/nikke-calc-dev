...
| 신데렐라 : 크리스탈 웨이브 | 스킬1 | **모드 전환 토글** — `저격 모드` weapon_change의 진입 조건(`event:full_reload` + `self_state:변경 준비`)과 해제 조건이 **동일**하다(`해제 조건 : 변경 준비 상태에서 최대 장탄 재장전 완료`). 스키마에 토글 표현이 없어 `note`로만 기록. `변경 준비`는 모든 풀리로드마다 부여(6초)되지만 `[마지막 탄환 공격 시 제거]`가 있어 **자연 탄약 고갈 재장전으로는 전환이 성립하지 않는다** — 탄이 남은 상태의 수동 재장전 전용. 유저 확인: 실전에서 MG 고정이 일반적이고 수동컨으로 저격 모드를 쓰기도 함 → 양쪽 다 모델링 대상. |
...
| 신데렐라 : 크리스탈 웨이브 | 스킬1 | `추가 효과 2 : 풀 차지 공격 시 탄환 소모 40발` — **실제 장탄은 1발씩 소모**하고, "아군이 소비한 탄의 총합"을 세는 스킬에 대해서만 40발로 집계된다(유저 확인). weapon_change 필드로 묶지 않고 독립 instant(`squad_ammo_consume_add`, `fixed_value: 40`, timing `full_charge_hit`)로 분리. 기본 MG 모드에서는 `full_charge_hit`가 없어 자연 게이팅. 일반 규칙은 GAMEPLAY §무기 메카닉에 승격. |
| 신데렐라 : 크리스탈 웨이브 | 스킬2 | `저격모드 적용 시` → `event:저격 모드`, `저격모드 해제 시` → `event:state_end:저격 모드`. ~~weapon_change는 `_active`에 등록되지 않아 event가 자동 발생하지 않는다~~ → **2026-07-28 해소**: weapon_change에 상태 정체성이 부여되어(`event:{name}` 브로드캐스트 + `event:state_end:{name}` + `self_state` 판정) 별도 발생 경로 없이 그대로 동작한다. `디스트로이`·`핀 포인트`·`self_state:저격 모드` 전부 이 경로로 살아난다. |
...
| 신데렐라 : 크리스탈 웨이브 | 스킬2 | `디스트로이`(파츠 대미지)와 `핀 포인트`(코어 대미지)는 **상호 배타** — 각 부여 clause에 상대 `remove_named_buff`가 붙어 있다. 동시 활성 시 파츠·코어 대미지 이중 계상이므로 네거티브 검증 대상. 파츠 모델 도입 전에는 `part_dmg_pct`가 죽은 값이라 저격 모드가 과소 평가된다. |
| 라플라스 : 얼티밋 히어로 | 스킬1 | **무기 변경 무기 유형·연사속도가 텍스트에 없다** — `[사용 무기 변경 : 일렉트릭 파워 풀 풀 차지]` 블록에 대미지·장탄·제거시점·관통만 있고 무기 유형이 빠져 있다. 유저 확인으로 **SMG, 통상 SMG와 동일한 초당 20발** 확정. 기본 무기(RL 차지형, 장탄 120·계수 2.5%)는 사실상 예열 축적 전용이고 실딜은 전부 SMG 모드에서 나온다(6.25%/s vs 189%/s). `weapon_delays.json`에 `post_fire_delay: 0` 예외 등록. |
